...
Let $\Sigma_X$ and $\Sigma_Y$ denote the set of finitely simple functions on $X$, $Y$ respectively.
...
for each $j = 1,\hdots,n$, $k = 1,\hdots,m$. Thus the function $F$ is well-defined on $\overline{S}$. Let $t \in \mathbb{R}$. For $p,p_0 \neq \infty$
